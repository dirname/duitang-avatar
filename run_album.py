...
start_album = int(input("Input start album id : "))
stop_album = int(input("Input stop album id : ")) + 1
for album_id in range(start_album, stop_album):
    start = 0  # 头像开始参数
    total = 1  # 总计头像页数
    ...
    while start < total:  # 判断是否采集完成
        images_list = list()
        url = "https://www.duitang.com/napi/blog/list/by_album/?album_id=" + str(album_id) + "&start=" + str(start)
        try:
            response = requests.get(url)
            obj = json.loads(response.text)
            if int(obj["status"] != 1):
                print("Album[%d] : %s" % (album_id, obj["message"]))
                break
